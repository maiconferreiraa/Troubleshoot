...
# --- FUNÇÃO AUXILIAR DE TEXTO PARA PDF (Movida para o escopo global) ---
def clean_text(text):
    """
    Garante que o texto é uma string e o codifica/decodifica usando latin-1 
    com substituição para compatibilidade com as fontes padrão do FPDF.
    """
...
def export_pdf():
    ...
    Corrigido o erro "Not enough horizontal space" usando uma largura fixa segura (180mm).
    """
    erros = fetch_all_erros()
    ...
    try:
        ...
        # Largura segura de conteúdo (180mm), que garante que o FPDF não quebre 
        # em cálculos de margem/largura completa.
        CONTENT_WIDTH = 180 
        LINE_HEIGHT = 6
        ...
        # Geração do título
        pdf.cell(0, 10, clean_text("Erros Cadastrados"), ln=True, align="C")
        pdf.ln(10)
        
        for r in erros:
            # Geração do cabeçalho de cada erro
            pdf.set_font("Arial", "B", 12)
            # Usa cell(0,...) seguido por ln() para o cabeçalho
            pdf.cell(0, LINE_HEIGHT, clean_text(f"ID: {r['id']} - Serviço: {r['servico']}")) 
            pdf.ln()

            # Geração do corpo do erro
            pdf.set_font("Arial", "", 11)
            
            # Usando largura fixa segura (CONTENT_WIDTH) para multi_cell para evitar o erro de espaço
            pdf.multi_cell(CONTENT_WIDTH, LINE_HEIGHT, clean_text(f"Erro: {r['erro']}"))
            
            # Linha que estava causando o erro, agora com CONTENT_WIDTH
            pdf.multi_cell(CONTENT_WIDTH, LINE_HEIGHT, clean_text(f"Solução: {r['solucao']}"))
            
            pdf.multi_cell(CONTENT_WIDTH, LINE_HEIGHT, clean_text(f"Criado por: {r['criado_por']}"))
        ...
        flash("❌ Erro ao gerar o PDF. Um problema de formatação de texto foi detectado. Por favor, tente novamente.", "danger")
        return redirect(url_for("dashboard"))
