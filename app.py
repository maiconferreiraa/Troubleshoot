...
def export_pdf():
    ...
    Adicionado try/except para capturar o erro exato do FPDF.
    """
    erros = fetch_all_erros()
    ...
    try:
        ...
        for r in erros:
            ...
            pdf.cell(0, 8, clean_text(f"ID: {r['id']} - Serviço: {r['servico']}")) 
            pdf.ln()

            pdf.set_font("Arial", "", 11)
            pdf.multi_cell(0, 6, clean_text(f"Erro: {r['erro']}"))
            pdf.multi_cell(0, 6, clean_text(f"Solução: {r['solucao']}"))
            pdf.multi_cell(0, 6, clean_text(f"Criado por: {r['criado_por']}"))
            pdf.ln(5)
        ...
        # Registra o traceback completo no log para que você possa ver o erro real do FPDF.
        app.logger.error(f"ERRO CRÍTICO na geração do PDF: {e}")
        app.logger.error(traceback.format_exc()) 
        # Retorna uma mensagem de erro que não seja 500 para o usuário
