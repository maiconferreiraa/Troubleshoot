...
def init_db():
    """Inicializa o banco de dados e cria as tabelas e colunas se não existirem (Migração de Schema)."""
    ...
    try:
        ...
        # 1. Tabela de Erros: Sempre garante que a tabela exista primeiro
        cur.execute("""
        ...
        # 1.1. MIGRAÇÃO: Adiciona a coluna user_email de forma segura (ALTER TABLE)
        try:
            # Tenta executar o ALTER TABLE
            cur.execute("""
                ALTER TABLE erros 
                ADD COLUMN user_email TEXT NOT NULL DEFAULT 'desconhecido'
            """)
            app.logger.info("✅ SUCESSO: Coluna 'user_email' adicionada à tabela 'erros'.")
        except pg_errors.DuplicateColumn:
            app.logger.info("ℹ️ INFO: Coluna 'user_email' já existe. Nenhuma alteração de schema necessária.")
        except Exception as e:
            # ❌ CORREÇÃO CRÍTICA: Se a migração falhar (ex: syntax, tipo de dado), a transação é ABORTADA.
            # É necessário forçar um ROLLBACK para limpar o estado e permitir que os próximos comandos continuem.
            app.logger.warning(f"⚠️ AVISO: Erro no ALTER TABLE. Executando ROLLBACK. Detalhes: {e}")
            conn.rollback() 
            # Após o rollback, é boa prática obter um novo cursor para garantir o estado limpo
            cur = conn.cursor() 

        # 2. Tabela de Usuários (Agora pode prosseguir, mesmo que o ALTER tenha falhado e dado rollback)
        cur.execute("""
        ...
        """)
        conn.commit()
        app.logger.info("✅ SUCESSO: Banco de dados PostgreSQL inicializado e schema verificado/corrigido.")
    except Exception as e:
...
def check_duplicate_erro(erro, servico, solucao):
    """
    Verifica se já existe um erro com o mesmo erro, serviço e solução.
    """
    try:
...
def fetch_all_erros():
    """Busca todos os erros cadastrados, incluindo a coluna user_email."""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT id, erro, servico, solucao, user_email FROM erros ORDER BY id DESC")
                return [dict(row) for row in cur.fetchall()] 
    except Exception as e:
        app.logger.error(f"Erro ao buscar erros: {e}")
        return []

def insert_erro(erro, servico, solucao, user_email):
    """Insere um novo erro no banco de dados, incluindo o usuário."""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO erros (erro, servico, solucao, user_email) VALUES (%s, %s, %s, %s)", 
                    (erro, servico, solucao, user_email)
                )
            conn.commit()
            return True
    except Exception as e:
...
def update_erro(id_, erro, servico, solucao):
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                # Não atualizamos o user_email, apenas o conteúdo
                cur.execute(
...
# --- Funções CRUD de Usuários e Tokens (Mantidas) ---
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not email or not password:
            ...
            # Render templates (assumido que existem)
...
# --- Dashboard e CRUD (Atualizado para inserir o usuário) ---
...
def dashboard():
    ...
    user_email = session.get("user") # O e-mail do usuário logado
    ...
    if request.method == "POST":
        ...
        if action == "cadastrar":
            ...
            if data:
                ...
                # Passa o e-mail do usuário logado para a função de inserção
                elif insert_erro(data["erro"], data["servico"], data["solucao"], user_email):
...
def export_csv():
    ...
    # Adicionando a nova coluna "Usuário" no cabeçalho
    writer.writerow(["ID", "Erro", "Serviço", "Solução", "Usuário"])
    # Adicionando o dado da nova coluna no corpo
    writer.writerows([[r["id"], r["erro"], r["servico"], r["solucao"], r["user_email"]] for r in erros])
...
def export_pdf():
    ...
    for r in erros:
        ...
        # Adicionando o usuário ao título do item
        pdf.cell(0, 8, clean_text(f"ID: {r['id']} - Servico: {r['servico']} - Usuário: {r['user_email'].split('@')[0]}")) 
        pdf.ln()
