...
# --- FUNÇÃO AUXILIAR DE TEXTO PARA PDF ---
def clean_text(text):
    """
...
def export_pdf():
    ...
    Corrigido o problema de texto cortado usando largura 0 (full width até a margem)
    para forçar a quebra de linha em multi_cell.
    """
    erros = fetch_all_erros()
    ...
    try:
        ...
        for r in erros:
            ...
            # Esta linha ocupa a largura total, e pdf.ln() move para o canto esquerdo da próxima linha
            ...
            # Usando largura 0 (zero) para multi_cell, que significa usar toda a largura
            # disponível restante até a margem direita, garantindo que o texto caiba sem ser cortado.
            pdf.multi_cell(0, LINE_HEIGHT, clean_text(f"Erro: {r['erro']}"))
            
            # Repetindo a largura 0 para a Solução
            pdf.multi_cell(0, LINE_HEIGHT, clean_text(f"Solução: {r['solucao']}"))
            
            pdf.multi_cell(0, LINE_HEIGHT, clean_text(f"Criado por: {r['criado_por']}"))
