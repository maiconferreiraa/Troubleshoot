...
def export_pdf():
    ...
    Corrigido o problema de largura zero na multi_cell adicionando set_x(l_margin).
    """
    erros = fetch_all_erros()
    ...
    try:
        ...
        for r in erros:
            ...
            # Imprime o cabeçalho do erro. ln=1 (padrão) move para a próxima linha
            ...
            # Força o cursor para o início da margem esquerda (X=l_margin) 
            # para garantir que multi_cell(0,...) tenha a largura total disponível.
            pdf.set_x(pdf.l_margin) 
            
            pdf.multi_cell(0, LINE_HEIGHT, clean_text(f"Erro: {r['erro']}"))
            
            # Não precisamos de set_x novamente aqui, pois multi_cell sempre move o cursor
            # para o início da linha seguinte após a quebra de texto.
